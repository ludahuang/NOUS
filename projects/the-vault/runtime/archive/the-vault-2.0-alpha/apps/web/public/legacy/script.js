...
const hostedVaultDataUrl = runtimeParams.get("vaultDataUrl") || "";
const hostedDiscoverQuery = runtimeParams.get("discoverQuery") || "";
const hostedSelectedNoteId = runtimeParams.get("selectedNoteId") || "";
const embedMode = runtimeParams.get("embed") || "";

if (embedMode === "graph") {
  document.body.classList.add("embed-graph-only");
}
...
function resolveObsidianLinks(drafts, externalTargets = []) {
  ...
  drafts.forEach((draft) => {
    ...
    registerLookup(relativePath, draft.id);
    registerLookup(relativePath.replace(/\.(md|markdown)$/i, ""), draft.id);
    registerLookup(relativePath.split("/").pop(), draft.id);
    aliases.forEach((alias) => registerLookup(alias, draft.id));
  });
  ...
  drafts.forEach((draft) => {
    ...
    const rawLinks = Array.isArray(draft.rawLinks)
      ? draft.rawLinks
...
async function loadHostedVaultGraph() {
  if (!hostedVaultDataUrl) {
    return false;
  }

  setGraphStatus("Loading persisted vault graph…");

  try {
    const response = await fetch(hostedVaultDataUrl, {
      credentials: "same-origin",
      cache: "no-store",
    });

    if (!response.ok) {
      throw new Error(`Vault graph request failed with ${response.status}`);
    }

    const payload = await response.json();
    const records = Array.isArray(payload.records) ? payload.records : [];
    state.activeVaultName = payload.vaultName || "Vault";
    state.pendingNewTitle = "";

    if (!records.length) {
      setGraphStatus(
        `Vault "${state.activeVaultName}" has no persisted notes yet. Loading live Wikipedia seed graph instead.`,
        "warning",
      );
      return false;
    }

    state.graphMode = "vault";
    state.wikipediaRecords = [];
    state.obsidianRecords = records.map((record) => ({
      ...record,
      aliases: Array.isArray(record.aliases) ? record.aliases : [],
      tags: Array.isArray(record.tags) ? record.tags : [],
      links: Array.isArray(record.links) ? record.links : [],
      rawLinks: Array.isArray(record.rawLinks)
        ? record.rawLinks
      ...
      relativePath: record.relativePath || "",
      folderPath: record.folderPath || "",
      folderName: record.folderName || record.folderPath?.split("/").pop() || "Notes",
      sourceType: record.sourceType || "obsidian",
      sourceLabel: record.sourceLabel || "Private vault note",
    }));
    state.activeSeedPageId =
      payload.selectedNoteId ||
      hostedSelectedNoteId ||
      state.obsidianRecords[0]?.id ||
      "";
    state.activeSeedTitle =
      state.obsidianRecords.find((record) => record.id === state.activeSeedPageId)?.title ||
      state.obsidianRecords[0]?.title ||
      state.activeVaultName;

    setSidebarTab("obsidian");
    rebuildActiveGraph(state.activeSeedPageId);
    setGraphStatus(
      `Loaded ${payload.noteCount || state.obsidianRecords.length} persisted note${(payload.noteCount || state.obsidianRecords.length) === 1 ? "" : "s"} and ${payload.edgeCount || 0} stored link${(payload.edgeCount || 0) === 1 ? "" : "s"} from "${state.activeVaultName}".`,
    );
    return true;
  } catch (error) {
    console.error(error);
    setGraphStatus(`Persistent vault graph failed to load: ${error.message}`, "error");
    return false;
  }
}
...
async function init() {
  ...
  if (hostedDiscoverQuery.trim()) {
    if (searchInput) {
      searchInput.value = hostedDiscoverQuery;
    }
    await loadTopicGraphFromQuery(hostedDiscoverQuery, hostedDiscoverQuery);
    return;
  }

  const loadedHostedVault = await loadHostedVaultGraph();
  if (!loadedHostedVault) {
