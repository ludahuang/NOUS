...
const agentDraftPanel = document.getElementById("agent-draft-panel");
const agentDraftMeta = document.getElementById("agent-draft-meta");
const agentDraftTitleInput = document.getElementById("agent-draft-title");
const agentDraftFolderInput = document.getElementById("agent-draft-folder");
const agentDraftBodyInput = document.getElementById("agent-draft-body");
const agentDraftSaveButton = document.getElementById("agent-draft-save");
const agentDraftCancelButton = document.getElementById("agent-draft-cancel");
...
function formatInlineList(values) {
  const items = uniqueValues(values.map((value) => normalizeWhitespace(value)).filter(Boolean));
  if (!items.length) {
    return "";
  }
...
function buildArticleSummary(page) {
  if (!page) {
    return "";
  }

  const baseSummary = normalizeWhitespace(
    page.extract ||
    page.description ||
    page.previewBlocks?.[0] ||
    stripMarkdown(page.markdown || ""),
  );

  const inheritedRelations = [];
  const mergedSourceNames = toArray(page.mergedSources || []).map((source) => String(source));

  if (mergedSourceNames.length > 1) {
    inheritedRelations.push(`Source relationships preserved from ${formatInlineList(mergedSourceNames)}.`);
  }

  if (page.sourceAnnotations?.length) {
    const importedPaths = page.sourceAnnotations
      .map((annotation) => annotation.relativePath || "")
      .filter(Boolean);
    const importedTags = page.sourceAnnotations.flatMap((annotation) =>
      toArray(annotation.tags).map((tag) => `#${String(tag).replace(/^#/, "")}`),
    );

    if (importedPaths.length) {
      inheritedRelations.push(`Imported note context inherited from ${formatInlineList(importedPaths)}.`);
    }

    if (importedTags.length) {
      inheritedRelations.push(`Imported link context carried through ${formatInlineList(importedTags.slice(0, 6))}.`);
    }
  }

  return normalizeWhitespace([baseSummary, ...inheritedRelations].filter(Boolean).join(" "));
}
...
function setAgentOpen(open) {
  ...
  if (assistantShell) {
    assistantShell.dataset.open = state.agentOpen ? "true" : "false";
  }
  if (!state.agentOpen) {
    closeAgentDraft(true);
  }
}
    ...
    .map(
      (message) => `
        <div class="agent-message ${message.role}">
          <span class="agent-message-label">${message.role === "user" ? "You" : "NOUS"}</span>
          ${message.html}
        </div>
      `,
    )
...
function closeAgentDraft(clearFields = false) {
  state.agentDraftIndex = -1;
  agentDraftPanel.hidden = true;

  if (clearFields) {
    agentDraftMeta.textContent = "Draft the missing bridge directly inside the assistant shell.";
    agentDraftTitleInput.value = "";
    agentDraftFolderInput.value = "";
    agentDraftBodyInput.value = "";
  }
}
...
function openAgentBridgeDraft(index) {
  ...
  state.agentDraftIndex = index;
  agentDraftPanel.hidden = false;
  agentDraftMeta.textContent = `Draft a bridge between "${suggestion.leftTitle}" and "${suggestion.rightTitle}" around "${suggestion.anchorTitle}".`;
  agentDraftTitleInput.value = bridgeDraft.title;
  agentDraftFolderInput.value = bridgeDraft.folderName;
  agentDraftBodyInput.value = bridgeDraft.markdown;
  window.setTimeout(() => {
    agentDraftTitleInput.focus();
    agentDraftTitleInput.select();
  }, 0);
  setGraphStatus(`Drafting bridge "${bridgeDraft.title}" inside the assistant shell.`);
}
...
async function commitLocalNoteRecord(record, { isNewNote }) {
  if (isNewNote) {
    ...
    if (existingWikipediaRecord) {
      ...
      return {
        status: "existing-wikipedia",
        pageId: existingWikipediaRecord.id,
        title: existingWikipediaRecord.title,
      };
    }
    ...
    if (topicResult.status === "ok") {
      state.pendingNewTitle = "";
      return {
        status: "existing-topic",
        topicResult,
      };
    }
  }

  const duplicate = findObsidianDuplicate(record, isNewNote ? "" : record.id);
  if (duplicate) {
    return {
      status: "duplicate",
      duplicate,
...
async function saveEditorNote() {
  ...
  const result = await commitLocalNoteRecord(record, { isNewNote });

  if (result.status === "existing-wikipedia") {
    closeNoteEditor(false);
    setSidebarTab("wikipedia");
    focusPage(result.pageId);
    setGraphStatus(
      `Used existing Wikipedia entry "${result.title}" instead of creating a duplicate local note.`,
    );
    return;
  }
  ...
  if (result.status === "lookup-failed") {
    setGraphStatus(
      `Wikipedia lookup failed for "${result.title}". Retry or rename the note to save it locally.`,
      "error",
    );
    return;
  }
  ...
  if (result.status === "existing-topic") {
    closeNoteEditor(false);
    applyDiscoveredWikipediaTopic(
      result.topicResult,
      `Used existing Wikipedia entry "${result.topicResult.seedRecord.title}" instead of creating a new local note.`,
    );
    return;
  }
  ...
  if (result.status === "duplicate") {
    setGraphStatus(
      `Duplicate local note blocked. "${result.duplicate.title}" already uses ${result.duplicate.relativePath}.`,
      "warning",
    );
    editorTitleInput.focus();
    return;
  ...
  closeNoteEditor(false);
  setSidebarTab("obsidian");
  setGraphStatus(`Saved local note "${result.record.title}" into the live graph.`);
}

async function saveAgentBridgeDraft() {
  const record = buildLocalNoteRecord({
    title: agentDraftTitleInput.value,
    folderName: agentDraftFolderInput.value,
    markdown: agentDraftBodyInput.value,
  });
  const result = await commitLocalNoteRecord(record, { isNewNote: true });
  ...
  if (result.status === "existing-wikipedia") {
    ...
    pushAgentMessage(
      "assistant",
      `<p>I reused the existing Wikipedia note <strong>${escapeHtml(result.title)}</strong> instead of creating a duplicate bridge.</p>`,
    );
    setGraphStatus(`Used existing Wikipedia entry "${result.title}" instead of creating a duplicate local note.`);
    return;
  }
  ...
  if (result.status === "existing-topic") {
    ...
    pushAgentMessage(
      "assistant",
      `<p>I reused the existing Wikipedia note <strong>${escapeHtml(result.topicResult.seedRecord.title)}</strong> instead of saving a duplicate bridge.</p>`,
    );
    return;
  }
  ...
  pushAgentMessage(
    "assistant",
    `<p>Saved bridge note <strong>${escapeHtml(result.record.title)}</strong> into the live graph.</p>`,
  );
  setGraphStatus(`Saved bridge note "${result.record.title}" into the live graph.`);
}
...
async function resetToDefaultVault() {
  clearSearchSuggestions();
  closeNoteEditor(true);
  setAgentOpen(false);
  searchInput.value = "";
  ...
  state.agentFocusQuery = "";
  clearGraphCanvas(
    "Restoring default vault…",
    "Clearing local additions and rebuilding the default Wikipedia connectome.",
  );
...
function bindEvents() {
  ...
  agentDraftSaveButton.addEventListener("click", () => {
    saveAgentBridgeDraft();
  });

  agentDraftCancelButton.addEventListener("click", () => {
    closeAgentDraft(true);
  });
